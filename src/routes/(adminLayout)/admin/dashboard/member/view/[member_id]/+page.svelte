<script lang="ts">
	...
	const currentPlan = $derived(gym_plans.find((p) => p?.plan_amount == member?.fee_pm) ?? null);
	...
	function formatDate(inputDate: string) {
		...
		return `${String(date.getDate()).padStart(2, '0')} ${getMonthName(date.getMonth())}, ${date.getFullYear()}`;
	}
</script>
...
<section class="h-full w-full">
	<div class="hms-container">
		...
		<section class="grid grid-cols-1 gap-10 text-lg">
			<div class="w-full">
				<h2 class="mb-4 flex max-w-[320px] flex-wrap items-center gap-2 border-b-2 border-secondary pb-2 text-2xl font-bold text-primary">
					Personal Info
			...
			<div class="w-full">
				...
				<div class="grid w-full grid-cols-1 gap-4 bg-base-300 p-4 text-base-content lg:grid-cols-2">
					...
					<p><strong>Months:</strong> {currentPlan?.plan_time} month</p>
					<p><strong>Batch:</strong> {member?.gym_time}</p>
					<p><strong>Joining Date:</strong> {formatDate(member?.joining_date)}</p>
					<p><strong>End Date:</strong> {formatDate(member?.end_date)}</p>
					<p><strong>Total Amount:</strong> {currentPlan?.plan_amount}</p>
					<p><strong>Amount Received:</strong> {member?.fee_received}</p>
					<p class="text-error"><strong>Amount Due:</strong> {member?.due_amount}</p>
				</div>
			</div>
		</section>
