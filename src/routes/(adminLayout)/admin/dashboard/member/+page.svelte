<script lang="ts">
	...
	let members: any[] = $state(data?.members ?? []);
		...
		<section class="mb-16">
			...
			<div class="grid w-full grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
				...
				<label class="form-control w-full">
					...
					<input type="text" placeholder="Type here" class="input input-bordered w-full" bind:value={filteraadhar_number} />
				</label>
				<label class="form-control w-full">
				...
				{#each filteredMembers as member}
					<div class="relative grid w-full grid-cols-1 gap-4 bg-base-200 p-4 shadow-md">
						...
						<div class="grid w-full grid-cols-1 gap-4 bg-base-200 p-4 md:grid-cols-2">
							...
							<p class="flex flex-col text-lg"><span>Plan Expiry:</span><strong>{formatDate(member?.end_date)}</strong></p>
						...
						<div class="flex w-full gap-4">
							...
							<button onclick={() => deleteMember(member?.id)} type="button" class="btn btn-error btn-sm">Delete</button>
						</div>
					</div>
				{/each}
			</div>
